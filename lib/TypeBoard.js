            ...
            for(let i = 0; i < rowArray.length; i++) {
                ...
                if(dataArray.length == 2) {
                    ...
                    if(tempArray.length > 1) { // スレタイに空白が含まれている場合lengthは３以上になります。
                        ...
                        const dateTime = new Date(Number(fileName.replace(/[^0-9]/g, '')*1000)); // Date型に合わせて秒をミリ秒換算にしてから使用しています。

                        tempThreadList[fileName] = new Thread(self, title, count, dateTime, fileName);
                    }
                }
            }
            // リストを比較し変更を反映
            for(let key in self.threadList) {
                if(tempThreadList[key]) {
                    delete tempThreadList[key];
                } else {
                    // threadListから削除してリソースを解放
                    delete self.threadList[key];
                }
            }
            for(let key in tempThreadList) {
                self.threadList[key] = tempThreadList[key];
            }
